...
import { AlertCircle, CheckCircle, XCircle, Clock, Calendar, Archive, ChevronDown, ChevronUp } from 'lucide-react-native';
...
export default function PlannerQCRequestsScreen() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (!user?.siteId) {
      console.log('❌ PLANNER QC QUERY - No siteId');
      setIsLoading(false);
      return;
    }

    console.log('🔍 PLANNER QC REALTIME - Setting up listener for siteId:', user.siteId);
    setIsLoading(true);
    ...
    const unsubscribe = onSnapshot(
      q,
      async (snapshot) => {
        console.log('📊 PLANNER QC REALTIME - Received', snapshot.docs.length, 'documents');
        const results: QCRequest[] = [];
        
        for (const docSnap of snapshot.docs) {
          const data = docSnap.data();
          console.log('📄 Request doc:', docSnap.id, '- Status:', data.status, '- Type:', data.type, '- SiteId:', data.siteId);
      ...
      (err) => {
        console.error('❌ PLANNER QC REALTIME - Error:', err);
    ...
    return () => {
      console.log('🔴 PLANNER QC REALTIME - Cleaning up listener');
      unsubscribe();
    };
  }, [user?.siteId]);
