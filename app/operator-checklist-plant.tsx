...
export default function OperatorChecklistPlantScreen() {
  ...
  const loadPlantAssetAndChecklist = useCallback(async () => {
    if (!plantAssetId || !user?.userId || !user?.masterAccountId) {
      setIsLoading(false);
      return;
    }
    ...
    try {
      setIsLoading(true);
      console.log('[OperatorChecklistPlant] Loading asset for plantAssetId:', plantAssetId);
      ...
      if (assetsSnapshot.empty) {
        console.log('[OperatorChecklistPlant] No plant asset found');
        Alert.alert('Not Found', 'Plant asset not found. Please scan a valid QR code.');
        router.back();
        return;
      }
      ...
      if (masterChecklist.length === 0) {
        console.log('[OperatorChecklistPlant] No checklist template found for asset');
      ...
      if (!checklistSnapshot.empty) {
        ...
        console.log('[OperatorChecklistPlant] Checklist already submitted for today');
      }

      console.log('[OperatorChecklistPlant] Loaded checklist with', freshChecklist.length, 'items');
    } catch (error) {
      console.error('[OperatorChecklistPlant] Error loading checklist:', error);
  ...
  const submitChecklist = async () => {
    ...
    try {
      ...
      console.log('[OperatorChecklistPlant] Checklist submitted successfully');
      ...
      console.error('[OperatorChecklistPlant] Error submitting checklist:', error);
      ...
      <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
        ...
        <View style={styles.submittedContainer}>
          ...
          <Text style={styles.submittedSubtitle}>
            You have already submitted your daily checklist for this asset today.
          </Text>
